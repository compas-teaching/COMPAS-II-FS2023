    ...
    for i,layer in enumerate(slicer.layers):
        if i% 3 == 0 and i>0:
            for j,path in enumerate(layer.paths):
                ...
                rotated_points = rotate_points(new_path_f, angle, [0,0,1], [0,0,0])
                rotated_points = [Point(*p) for p in rotated_points]

                # compas_rotated_points = []
                # for p in rotated_points:
                #     compas_rotated_points.append(Point(*p))
                # rotated_points = compas_rotated_points

                angle += increment 

                layer.paths[j] = Path(rotated_points, is_closed=path.is_closed)
